...
def train(args):
    ...
    while epoch < args.epochs:
        ...
        for i, data in enumerate(dataloader, 0):
            real_images = data[0]
            valid = torch.FloatTensor(real_images.shape[0], 1).fill_(1.0)
            fake = torch.FloatTensor(real_images.shape[0], 1).fill_(0.0)
            z = torch.FloatTensor(
                np.random.normal(0, 1, (real_images.shape[0], args.latent_dim))
            )
            ...
            if epoch % args.sample_interval == 0 and i % (len(dataloader) / 5) == 0:
                save_image(
                    gen_imgs.data[0],
                    "./images/{}_{}.png".format(
                        str(epoch).zfill(len(str(args.epochs))),
